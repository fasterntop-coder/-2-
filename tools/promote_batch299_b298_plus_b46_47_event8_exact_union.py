...
PARENT_STATUS='PASS_BATCH298_B297_PLUS_B48_49_EVENT5_EXACT_UNION'
SUCCESS='PASS_BATCH299_B298_PLUS_B46_47_EVENT8_EXACT_UNION'
LOGICAL_ROWS_SHA='9cd5f874c9dafa87dd4350d3fd03aae937bd0bf7b049e96aae883efc5c0c60fc'
...
def main():
 ...
 if pr.get('status')!=PARENT_STATUS or pr.get('output_sha256')!=parent_sha:
  raise SystemExit('FAIL Batch298 parent report/SHA binding')

 manifest_sha=shaf(a.union_manifest)
 m=json.loads(a.union_manifest.read_text(encoding='utf-8'))
 rows=m.get('replacement_files')
 if m.get('format')!='ST2-CD1-batch299-b298-plus-b46-47-event8-exact-union-v1': raise SystemExit('FAIL manifest format')
 if m.get('parent_batch')!=298 or m.get('replacement_assets')!=8: raise SystemExit('FAIL manifest header')
 if m.get('records_reviewed')!=38 or m.get('records_translated')!=38 or m.get('control_preserved_records')!=0: raise SystemExit('FAIL frozen record totals')
 if m.get('guessed_payload_bytes')!=0: raise SystemExit('FAIL guessed bytes policy')
 if not isinstance(rows,list) or len(rows)!=8: raise SystemExit('FAIL manifest schema/count')
 ...
 if {int(r['source_batch']) for r in rows}!={46,47}: raise SystemExit('FAIL source batch set')
 if len({r['iso_path'] for r in rows})!=8: raise SystemExit('FAIL duplicate asset path')

 parent=a.parent_bin.read_bytes(); out=bytearray(parent); expected={}
 wanted={r['replacement_sha256'].lower() for r in rows}
 with tempfile.TemporaryDirectory(prefix='st2_b299_') as td:
  ...
  audit=[]
  for r in rows:
   path=r['iso_path']; lba=int(r['lba']); size=int(r['size']); source_batch=int(r['source_batch'])
   ...
   audit.append({'source_batch':source_batch,'iso_path':path,'lba':lba,'size':size,'parent_asset_sha256':cur_sha,'final_asset_sha256':final,'state':state,'reextraction':'PASS'})
 ...
 rep={'batch':299,'status':SUCCESS,'parent_batch':298,'parent_sha256':parent_sha,'output_sha256':output_sha,'pristine_reference_sha256':PRISTINE_SHA,'union_manifest_sha256':manifest_sha,'logical_rows_sha256':LOGICAL_ROWS_SHA,'source_batches':[46,47],'replacement_assets':8,'records_reviewed':38,'records_translated':38,'control_preserved_records':0,'guessed_payload_bytes':0,'asset_reextraction':'8/8 PASS','expected_write':[expected[L] for L in sorted(expected)],'changed_raw_sectors':len(actual),'changed_lbas':actual,'changed_sector_accounting':'PASS','changed_sector_edc_ecc':f'{len(actual)}/{len(actual)} PASS','asset_audit':audit}
 a.report.parent.mkdir(parents=True,exist_ok=True); a.report.write_text(json.dumps(rep,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(SUCCESS); print('output_sha256='+output_sha); print('event_assets=8/8'); print('records_reviewed=38 translated=38 control_preserved=0'); print(f'changed_raw_sectors={len(actual)}')
