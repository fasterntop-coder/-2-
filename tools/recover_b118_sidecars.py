...
def workbook_rows(path: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    ...
    for ws in wb.worksheets:
        ...
        if {x.lower() for x in AUDIT_REQUIRED}.issubset(lowered):
            by_kind["audit"] = rows
        elif "decoded_korean" in lowered and "bank" in lowered and "record" in lowered:
            by_kind["reverse"] = rows
    if "reverse" not in by_kind or "audit" not in by_kind:
        raise ValueError("required Reverse Decode / Record Audit sheets not found")
    return by_kind["reverse"], by_kind["audit"]
...
def validate(rows: list[dict[str, str]], expected_rows: int, split: dict[str, int], required: set[str], kind: str) -> list[dict[str, str]]:
    if len(rows) != expected_rows:
        raise ValueError(f"{kind}: rows {len(rows)} != {expected_rows}")
    normalized = [keymap(row) for row in rows]
    if not normalized or not required.issubset(normalized[0]):
        raise ValueError(f"{kind}: missing columns {sorted(required - set(normalized[0] if normalized else {}))}")
    counts = Counter(row["bank"] for row in normalized)
    if dict(counts) != split:
        raise ValueError(f"{kind}: bank split {dict(counts)} != {split}")
    ...
    for row in normalized:
        ...
        seen.add(key)
        if not 0 <= record <= 228:
            raise ValueError(f"{kind}: record out of range {key}")
        for field in required:
            if not row.get(field, ""):
                raise ValueError(f"{kind}: blank {field} at {key}")
        for field in ("source_record_sha256", "candidate_record_sha256") & required:
            value = row[field].lower()
            if len(value) != 64 or any(c not in "0123456789abcdef" for c in value):
                raise ValueError(f"{kind}: invalid {field} at {key}")
    return normalized
...
def recover(root: Path, output: Path) -> dict[str, object]:
    ...
    reverse_keys = {(r["bank"], int(r["record"])) for r in reverse}
    audit_keys = {(r["bank"], int(r["record"])) for r in audit}
    if not reverse_keys.issubset(audit_keys):
        raise ValueError("reverse rows are not a subset of audited records")
    output.mkdir(parents=True, exist_ok=True)
    rpath, apath = output / REVERSE_NAME, output / AUDIT_NAME
    result = {
        "status": "PASS_B118_SIDECARS_RECOVERED",
        "source": source,
        "reverse_rows": len(reverse), "audit_rows": len(audit),
        "reverse_sha256": write_csv(rpath, reverse),
        "audit_sha256": write_csv(apath, audit),
        "reverse_path": str(rpath), "audit_path": str(apath),
    }
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        def emit(path: Path, rows: int, sys_count: int, reverse: bool) -> None:
            ...
            with path.open("w", encoding="utf-8", newline="") as f:
                ...
                for i in range(rows):
                    bank = "SYSTEM" if i < sys_count else "SYS14"; rec = i if i < sys_count else i - sys_count
                    row = {"bank": bank, "record": rec}
        ...
        result = recover(root, out)
        return {"status": "PASS" if result["status"].startswith("PASS") else "FAIL"}


def main() -> int:
    p = argparse.ArgumentParser(); sub = p.add_subparsers(dest="cmd", required=True)
    r = sub.add_parser("recover"); r.add_argument("root", type=Path); r.add_argument("--output-dir", type=Path, default=Path("output/B118_SIDECARS"))
    sub.add_parser("selftest"); args = p.parse_args()
