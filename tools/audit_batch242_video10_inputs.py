...
GATE_FORMAT = "ST2-CD1-BATCH241-VIDEO9-RECOVERY-GATE-v2"
...
def load_gate(path: Path) -> dict:
    ...
    if len(obj.get("direct_candidates", [])) != 6:
    ...
    if obj.get("correction", {}).get("excluded_already_promoted_asset") != "SK2MV_30.CAK":
        raise SystemExit("Batch240 overlap correction missing")
    if any(x.get("asset") == "SK2MV_30.CAK" for x in obj.get("direct_candidates", [])):
        raise SystemExit("SK2MV_30.CAK is already physically promoted in Batch240")
    return obj
    ...
    try:
        with zipfile.ZipFile(path, "r") as zf:
            for info in zf.infolist():
                if not info.is_dir() and Path(info.filename).name.lower() == expected_name and info.file_size == spec["size"]:
...
def audit_direct(base: Path, spec: dict) -> dict:
    ...
    row = {"kind":"direct_candidate","name":spec["asset"],"expected_sha256":spec["replacement_sha256"],"expected_size":spec["size"],"present":path.is_file(),"ready":False}
    if not path.is_file():
        row["status"] = "MISSING"
        return row
    row["actual_size"] = path.stat().st_size
    if row["actual_size"] != spec["size"]:
        row["status"] = "SIZE_MISMATCH"
        return row
    row["actual_sha256"] = sha256_file(path)
    if row["actual_sha256"] != spec["replacement_sha256"]:
        row["status"] = "SHA256_MISMATCH"
        return row
    row["status"] = "PASS"
    row["ready"] = True
...
def audit_disc(path: Path | None, label: str, expected_sha256: str) -> dict:
    row = {"kind":"disc","name":label,"expected_sha256":expected_sha256,"required_for_promotion":True,"ready":False}
    if path is None:
        row["present"] = False
    ...
    if not path.is_file():
        ...
        return row
    row["actual_sha256"] = sha256_file(path)
    if row["actual_sha256"] != expected_sha256:
        row["status"] = "SHA256_MISMATCH"
        return row
    row["status"] = "PASS"
    row["ready"] = True
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Read-only Batch242 preflight for the remaining Video9; SK2MV_30 is already in Batch240.")
    ...
    ap.add_argument("--result", type=Path, default=Path("BATCH242_VIDEO9_PREFLIGHT.json"))
    args = ap.parse_args()
    ...
    discs = [audit_disc(args.pristine,"pristine_disc1",PRISTINE_SHA), audit_disc(args.parent,"batch240_parent",PARENT_SHA)]
    ...
    result = {
        ...
        "payloads":{"required":len(payload_rows),"ready":sum(1 for r in payload_rows if r["ready"]),"missing":[r["name"] for r in payload_rows if r["status"]=="MISSING"],"rejected":[{"name":r["name"],"status":r["status"]} for r in payload_rows if r["status"] not in {"PASS","MISSING"}],"items":payload_rows},
        "discs":discs,
        "gates":{"all_payload_sha256_or_trusted_zip_contract":payload_ready,"pristine_sha256":discs[0]["ready"],"parent_sha256":discs[1]["ready"],"expected_write":"DEFERRED_TO_BATCH242_INTEGRATOR","edc_ecc":"DEFERRED_TO_BATCH242_INTEGRATOR","whole_asset_reextraction":"DEFERRED_TO_BATCH242_INTEGRATOR"},
        "next_command": ("python tools/recover_batch241_video10.py --archive-dir <archives> --candidate-dir <candidates> --output-dir BATCH241_VIDEO9_RECOVERED && python tools/integrate_batch241_video10_batch242.py --pristine <pristine.bin> --parent <batch240.bin> --candidate-dir BATCH241_VIDEO9_RECOVERED --manifest BATCH241_VIDEO9_RECOVERED/BATCH241_VIDEO9_CONSOLIDATED_MANIFEST.json") if payload_ready and disc_ready else None,
    }
