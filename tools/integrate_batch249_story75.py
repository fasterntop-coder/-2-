...
def main()->int:
 ap=argparse.ArgumentParser(description='Batch249 exact 75-story promotion onto exact Batch247')
 ...
 ap.add_argument('--manifest',type=Path,default=Path('manifests/CD1_BATCH249_STORY75_MEGA_PROMOTION.json'));ap.add_argument('--output',type=Path,default=Path('Sakura_Taisen_2_Disc1_B249_C2FIX_STATIC58_STORY75_KO.bin'));ap.add_argument('--result',type=Path,default=Path('BATCH249_RESULT.json'))
 a=ap.parse_args();m=json.loads(a.manifest.read_text(encoding='utf-8'));xs=m.get('replacement_files',[])
 if m.get('format')!='ST2-CD1-BATCH249-STORY75-MEGA-PROMOTION-v1' or m.get('asset_count')!=75 or len(xs)!=75:raise SystemExit('manifest format/cardinality mismatch')
 ...
 for x in xs:
  ...
  if footprint&ls:raise SystemExit(f'story75 footprint collision: {n}')
  footprint|=ls;per[n]={'lba':x['lba'],'size':x['size'],'footprint_sectors':len(ls)}
 old=diffs(a.pristine,a.parent)
 if old&footprint:raise SystemExit(f'Batch247 overlaps story75 footprint at LBA {min(old&footprint)}')
 ...
 try:
  ...
  if not changed<=footprint:raise ValueError('change outside approved story75 footprint')
  ...
  result={'batch':249,'status':'PASS_B247_STATIC58_PLUS_STORY75_EXECUTABLE_CANDIDATE','parent_batch':247,'parent_sha256':PARENT_SHA,'story_assets_promoted':75,'output_sha256':shaf(a.output),'approved_footprint_sectors':len(footprint),'changed_sectors':len(changed),'parent_overlap':0,'outside_footprint_changes':0,'changed_sector_edc_ecc':f'{len(changed)}/{len(changed)} PASS','whole_asset_reextraction':'75/75 PASS','expected_write_records':len(expected),'safety':{'guessed_bytes':False,'exact_candidate_sha256':True,'exact_pristine_source_sha256':True,'expected_write':True,'changed_sector_accounting':True},'per_asset':per}
