...
PARENT_STATUS='PASS_BATCH299_B298_PLUS_B46_47_EVENT8_EXACT_UNION'
SUCCESS='PASS_BATCH300_B299_PLUS_B41_44_EVENT9_EXACT_UNION'
LOGICAL_ROWS_SHA='fab994d419d16a704fb9018ad272b1ca67aa367f171f42b0314436e32369401d'
...
def main():
 ...
 if pr.get('status')!=PARENT_STATUS or pr.get('output_sha256')!=parent_sha:
  raise SystemExit('FAIL Batch299 parent report/SHA binding')

 manifest_sha=shaf(a.union_manifest)
 m=json.loads(a.union_manifest.read_text(encoding='utf-8'))
 rows=m.get('replacement_files')
 if m.get('format')!='ST2-CD1-batch300-b299-plus-b41-44-event9-exact-union-v1': raise SystemExit('FAIL manifest format')
 if m.get('parent_batch')!=299 or m.get('replacement_assets')!=9: raise SystemExit('FAIL manifest header')
 if m.get('records_reviewed')!=92 or m.get('records_translated')!=90 or m.get('control_preserved_records')!=2: raise SystemExit('FAIL frozen record totals')
 if m.get('guessed_payload_bytes')!=0: raise SystemExit('FAIL guessed bytes policy')
 if not isinstance(rows,list) or len(rows)!=9: raise SystemExit('FAIL manifest schema/count')
 ...
 if {int(r['source_batch']) for r in rows}!={41,42,43,44}: raise SystemExit('FAIL source batch set')
 if len({r['iso_path'] for r in rows})!=9: raise SystemExit('FAIL duplicate asset path')

 parent=a.parent_bin.read_bytes(); out=bytearray(parent); expected={}
 wanted={r['replacement_sha256'].lower() for r in rows}
 with tempfile.TemporaryDirectory(prefix='st2_b300_') as td:
 ...
 rep={'batch':300,'status':SUCCESS,'parent_batch':299,'parent_sha256':parent_sha,'output_sha256':output_sha,'pristine_reference_sha256':PRISTINE_SHA,'union_manifest_sha256':manifest_sha,'logical_rows_sha256':LOGICAL_ROWS_SHA,'source_batches':[41,42,43,44],'replacement_assets':9,'records_reviewed':92,'records_translated':90,'control_preserved_records':2,'guessed_payload_bytes':0,'asset_reextraction':'9/9 PASS','expected_write':[expected[L] for L in sorted(expected)],'changed_raw_sectors':len(actual),'changed_lbas':actual,'changed_sector_accounting':'PASS','changed_sector_edc_ecc':f'{len(actual)}/{len(actual)} PASS','asset_audit':audit}
 a.report.parent.mkdir(parents=True,exist_ok=True); a.report.write_text(json.dumps(rep,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(SUCCESS); print('output_sha256='+output_sha); print('event_assets=9/9'); print('records_reviewed=92 translated=90 control_preserved=2'); print(f'changed_raw_sectors={len(actual)}')
