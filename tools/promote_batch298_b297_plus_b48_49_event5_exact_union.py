...
PARENT_STATUS='PASS_BATCH297_B296_PLUS_B51_EVENT9_EXACT_UNION'
SUCCESS='PASS_BATCH298_B297_PLUS_B48_49_EVENT5_EXACT_UNION'
LOGICAL_ROWS_SHA='a9afe86394baccb90be99bb97da72054e4ae95fcdd7c9243a3129bc5ff98e03e'
...
def main():
 ...
 if pr.get('status')!=PARENT_STATUS or pr.get('output_sha256')!=parent_sha:
  raise SystemExit('FAIL Batch297 parent report/SHA binding')

 manifest_sha=shaf(a.union_manifest)
 m=json.loads(a.union_manifest.read_text(encoding='utf-8'))
 rows=m.get('replacement_files')
 if m.get('format')!='ST2-CD1-batch298-b297-plus-b48-49-event5-exact-union-v1': raise SystemExit('FAIL manifest format')
 if m.get('parent_batch')!=297 or m.get('replacement_assets')!=5: raise SystemExit('FAIL manifest header')
 if m.get('records_reviewed')!=68 or m.get('records_translated')!=67 or m.get('control_preserved_records')!=1: raise SystemExit('FAIL frozen record totals')
 if m.get('guessed_payload_bytes')!=0: raise SystemExit('FAIL guessed bytes policy')
 if not isinstance(rows,list) or len(rows)!=5: raise SystemExit('FAIL manifest schema/count')
 ...
 if {int(r['source_batch']) for r in rows}!={48,49}: raise SystemExit('FAIL source batch set')
 if len({r['iso_path'] for r in rows})!=5: raise SystemExit('FAIL duplicate asset path')

 parent=a.parent_bin.read_bytes(); out=bytearray(parent); expected={}
 wanted={r['replacement_sha256'].lower() for r in rows}
 with tempfile.TemporaryDirectory(prefix='st2_b298_') as td:
 ...
 rep={'batch':298,'status':SUCCESS,'parent_batch':297,'parent_sha256':parent_sha,'output_sha256':output_sha,'pristine_reference_sha256':PRISTINE_SHA,'union_manifest_sha256':manifest_sha,'logical_rows_sha256':LOGICAL_ROWS_SHA,'source_batches':[48,49],'replacement_assets':5,'records_reviewed':68,'records_translated':67,'control_preserved_records':1,'guessed_payload_bytes':0,'asset_reextraction':'5/5 PASS','expected_write':[expected[L] for L in sorted(expected)],'changed_raw_sectors':len(actual),'changed_lbas':actual,'changed_sector_accounting':'PASS','changed_sector_edc_ecc':f'{len(actual)}/{len(actual)} PASS','asset_audit':audit}
 a.report.parent.mkdir(parents=True,exist_ok=True); a.report.write_text(json.dumps(rep,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(SUCCESS); print('output_sha256='+output_sha); print('event_assets=5/5'); print('records_reviewed=68 translated=67 control_preserved=1'); print(f'changed_raw_sectors={len(actual)}')
