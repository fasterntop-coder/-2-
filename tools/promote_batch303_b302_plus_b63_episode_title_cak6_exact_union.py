...
PARENT_STATUS='PASS_BATCH302_B301_PLUS_B118_BATTLE58_EXACT_UNION'
SUCCESS='PASS_BATCH303_B302_PLUS_B63_EPISODE_TITLE_CAK6_EXACT_UNION'
FORMAT='ST2-CD1-batch303-b302-plus-b63-episode-title-cak6-exact-union-v1'
EXPECTED_ASSETS=6
...
def validate_assets(m:dict)->list[dict]:
 ...
 seen_name=set();seen_lba=set()
 for r in rows:
  for k in ('asset','lba','size','source_sha256','candidate_sha256','audio_sha256'):
   if k not in r:raise SystemExit(f'FAIL missing {k}')
  if r['asset'] in seen_name or int(r['lba']) in seen_lba:raise SystemExit('FAIL duplicate asset/LBA')
  seen_name.add(r['asset']);seen_lba.add(int(r['lba']))
  for k in ('source_sha256','candidate_sha256','audio_sha256'):
   v=str(r[k]).lower()
   if len(v)!=64 or any(c not in '0123456789abcdef' for c in v):raise SystemExit(f'FAIL SHA field {r["asset"]} {k}')
  if int(r['size'])<=0:raise SystemExit(f'FAIL size {r["asset"]}')
 return rows

def main():
 ap=argparse.ArgumentParser(description='Promote exact Batch63 six Korean episode-title CAK candidates onto Batch302.')
 ...
 if pr.get('status')!=PARENT_STATUS or pr.get('output_sha256')!=parent_sha:raise SystemExit('FAIL Batch302 parent report/SHA binding')
 m=json.loads(a.union_manifest.read_text(encoding='utf-8'))
 if m.get('format')!=FORMAT or m.get('parent_batch')!=302 or m.get('batch')!=303:raise SystemExit('FAIL B303 manifest format/header')
 hs=m.get('historical_source',{})
 if hs.get('status')!='PASS_OFFLINE_CANDIDATE' or hs.get('episode_title_cards')!='6/6' or hs.get('adx_audio_identical')!='6/6' or hs.get('source_size_preserved')!='6/6':raise SystemExit('FAIL B63 historical gate')
 pol=m.get('integration_policy',{})
 if pol.get('guessed_payload_bytes')!=0 or not pol.get('expected_write') or not pol.get('changed_lba_accounting') or not pol.get('changed_sector_edc_ecc'):raise SystemExit('FAIL integration policy')
 ...
 with tempfile.TemporaryDirectory(prefix='st2_b303_') as td:
  ...
  for r in rows:
   ...
   audit.append({'asset':asset,'lba':lba,'size':size,'parent_asset_sha256':cur,'final_asset_sha256':final,'state':state,'reextraction':'PASS'})
 actual=[]
 ...
 rep={'batch':303,'status':SUCCESS,'parent_batch':302,'parent_sha256':parent_sha,'output_sha256':output_sha,'pristine_reference_sha256':PRISTINE_SHA,'union_manifest_sha256':shaf(a.union_manifest),'replacement_assets':6,'episode_title_cak':'6/6','hardware_validation':'PENDING','guessed_payload_bytes':0,'asset_reextraction':'6/6 PASS','expected_write':[expected[L] for L in sorted(expected)],'changed_raw_sectors':len(actual),'changed_lbas':actual,'changed_sector_accounting':'PASS','changed_sector_edc_ecc':f'{len(actual)}/{len(actual)} PASS','asset_audit':audit}
 a.report.parent.mkdir(parents=True,exist_ok=True);a.report.write_text(json.dumps(rep,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(SUCCESS);print('output_sha256='+output_sha);print('episode_title_cak=6/6');print(f'changed_raw_sectors={len(actual)}');print('guessed_payload_bytes=0')
