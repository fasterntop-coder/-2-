...
PARENT_STATUS='PASS_BATCH296_B295_PLUS_B52_54_EVENT45_EXACT_UNION'
SUCCESS='PASS_BATCH297_B296_PLUS_B51_EVENT9_EXACT_UNION'
LOGICAL_ROWS_SHA='2482d9e3ac18c295e5247eabc26ac6d8233e8c040e32aede686c26c563f847d4'
...
def main():
 ...
 if pr.get('status')!=PARENT_STATUS or pr.get('output_sha256')!=parent_sha:
  raise SystemExit('FAIL Batch296 parent report/SHA binding')

 manifest_sha=shaf(a.union_manifest)
 m=json.loads(a.union_manifest.read_text(encoding='utf-8'))
 rows=m.get('replacement_files')
 if m.get('format')!='ST2-CD1-batch297-b296-plus-b51-event9-exact-union-v1': raise SystemExit('FAIL manifest format')
 if m.get('parent_batch')!=296 or m.get('replacement_assets')!=9: raise SystemExit('FAIL manifest header')
 ...
 if {int(r['source_batch']) for r in rows}!={51}: raise SystemExit('FAIL source batch set')
 ...
 with tempfile.TemporaryDirectory(prefix='st2_b297_') as td:
  ...
  for r in rows:
   ...
   audit.append({'source_batch':51,'iso_path':path,'lba':lba,'size':size,'parent_asset_sha256':cur_sha,'final_asset_sha256':final,'state':state,'reextraction':'PASS'})
 ...
 rep={'batch':297,'status':SUCCESS,'parent_batch':296,'parent_sha256':parent_sha,'output_sha256':output_sha,'pristine_reference_sha256':PRISTINE_SHA,'union_manifest_sha256':manifest_sha,'logical_rows_sha256':LOGICAL_ROWS_SHA,'source_batches':[51],'replacement_assets':9,'records_reviewed':105,'records_translated':104,'control_preserved_records':1,'guessed_payload_bytes':0,'asset_reextraction':'9/9 PASS','expected_write':[expected[L] for L in sorted(expected)],'changed_raw_sectors':len(actual),'changed_lbas':actual,'changed_sector_accounting':'PASS','changed_sector_edc_ecc':f'{len(actual)}/{len(actual)} PASS','asset_audit':audit}
 a.report.parent.mkdir(parents=True,exist_ok=True); a.report.write_text(json.dumps(rep,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(SUCCESS); print('output_sha256='+output_sha); print('event_assets=9/9'); print('records_reviewed=105 translated=104 control_preserved=1'); print(f'changed_raw_sectors={len(actual)}')
